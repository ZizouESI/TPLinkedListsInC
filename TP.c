...
annuaire * suppression_hotel(char nom_hotel[26] ,char ville_hotel[26],annuaire *tete)
{
    ...
    if (trouv1 == vrai)
    {
        ...
        if ((q->suiv)== (NULL))
        {
            if ((strcmp(nom_hotel,q->nom) == 0 ) &&  (strcmp(ville_hotel,q->ville) == 0 ))
            {
             q->tete_ch=liberer_toute_les_chambres(q->tete_ch);
             free(q);
             tete=p->suiv;
             free(p);
            }
        ...
        {
            if ((strcmp(q->nom,nom_hotel) == 0)&&(strcmp(q->ville,ville_hotel) == 0 ))
            {
            ...
            else
            {
                ...
                {
                    color(15,12);
                    printf("L'hotel que vous rechercher n'existe pas , verifier a nouveau le nom !");
                    color(15,0);
                    printf("\n");
                }
                else
    ...
    {
        color(15,12);
        printf("Desole , les hotels qui commancent par le charactere %c n'existent pas !",ch);
        color(15,0);
        printf("\n");
    }
    return tete;
...
{
    boolean trouv1=faux,trouv2=faux,trouv3=faux,trouv4=faux;
    ...
    else  // choix = 2 ou 3
    {
        ...
        else  // choix ==3
        {
            if (choix==3)
            {
                ...
                color(15,6);
                printf("Le nom de l'hotel est : %s \n",l->nom);
                color(15,0);
                printf("    La ville de l'hotel est : %s \n",l->ville);
...
annuaire * charger_reservation_depuis_un_fichier(annuaire *tete)
{
    ...
    int type,jd,md,ad,jf,mf,af,n;
    FILE *fic = fopen("chargement.txt","r");

    while (!feof(fic))
    {
        fgets(ligne,256,fic);
        sscanf(ligne," %s # %s # %s # %d # %d # %d # %d # %d # %d # %d # %d #",nom_hotel,ville,nom_reservateur,&n,&type,&jd,&md,&ad,&jf,&mf,&af);
        tete= reservation_auto(tete ,nom_reservateur,nom_hotel,ville,n,type,jd,md,ad,jf,mf,af);
    }
    fclose(fic);
    return tete;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
annuaire * reservation_auto(annuaire * tete,char nom_reservateur[26],char nom_hotel[26], char nom_ville[26],int n,int type,int jj,int mm,int aaaa,int jour,int mois,int annee)
{
        ...
        while (tete != NULL)
        {
            ...
            while ( h!=NULL )
            {
                ...
                while (c != NULL)
                {
                    if (c->statut == 1)
                    {
                        fprintf(fic," %s # %s # %s # %d # %d # %d # %d # %d # %d # %d # %d #\n",h->nom,h->ville,c->nom,c->nemuro,c->type,c->date_debut.jour,c->date_debut.mois,c->date_debut.annee,c->date_fin.jour,c->date_fin.mois,c->date_fin.annee);
                    }
                    c=c->suiv;
                }
                h=h->suiv;
    ...
    while (( p != NULL) && (trouv == faux))
    {
        ...
        while ((h != NULL) && (trouv == faux))
        {
            ...
            while ((c != NULL)&&(trouv == faux))
            {
                ...
                {
                    color(15,4);
                    printf("Les donnee rechercher par rapport a la reservation du %s sont les suivant :",nom_reservateur);
                    color(15,0);
                    printf("\n");
